...
int main()
{
	...
	// map 에 들어가는 pair 도 2개짜리 tuple 이므로 사용가능
	...
	auto [x, y] = arr;
	auto& [xr, yr] = arr;
	std::cout << "x : " << x << " / y : " << y << std::endl;	// x: 1 / y : 2
	std::cout << "xr : " << xr << " / yr : " << yr << std::endl;	// xr: 1 / yr : 2
	
	const auto [a, b] = f();
	std::cout << "a : " << a << " / b : " << b << std::endl;	// a : 1 / b : 2.3
	a = -2;	// mutable 은 const 여도 수정이 가능함
	// b = -2;	// 컴파일 오류 : b 가 const 임
	std::cout << "a : " << a << " / b : " << b << std::endl;	// a : -2 / b : 2.3

	// 구조적 바인딩은 완벽한 전달이 수행됨
	float f{};
	char c{};
	int i{};
	std::tuple<float&, char&&, int> tpl(f, std::move(c), i);
	const auto& [e1, e2, e3] = tpl;
	// 위 경우
	// decltype(e1) = float&
	// decltype(e2) = char&&
	// decltype(e3) = int
	...
	/*
		- 구조적 바인딩 Structured binding
			- C++ 17 에서 새로 추가된 기능
			- tuple 을 이용하여 여러 변수들을 한번에 초기화함
			- 위에 나온 코드들 말고도 다양한 구조적 바인딩 문법들이 있음 https://en.cppreference.com/w/cpp/language/structured_binding 를 참고

			- 가장 기본적으로 구조적 바인딩은 아래와 같이 사용함

					attr ﻿(optional) cv-auto ref-qualifier ﻿(optional) [ identifier-list ] = expression ;
					속성(선택) const/volatil auto 레퍼런스(선택) [식별자] = 표현식;

				-> 이를 따라 대로 아래처럼 사용하면 됨

					auto [age, name, is_male] = student; -> student 가 GetStudent() 로 받아온 학생 정보를 담고있는 tuple 임\

				-> 위 경우 age, name, is_male 에 student 튜플 안의 내용들이 순서대로 들어감

			- 튜플이 아닌 것들도 사용 가능함.

					int a[2] = {1, 2};
					auto [x,y] = a;	-> x = 1, y = 2 로 들어감

				- 그 밖에 데이터 멤버들이 정의된 구조체도 가능

			- 구조적 바인딩을 사용할 때는 모든 원소들을 받아와야함
				-> 즉, std::tuple_size<T>::value 와 [] 안의 식별자(변수이름) 의 갯수가 동일해야 함
				-> 중간에 몇개 빼고 가져온다거나 하는 등의 방법은 불가능함
	*/
}